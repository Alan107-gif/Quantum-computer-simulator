...
class QuantumRegister:
    ...
    def load_binary(self, binary_str):
        """
        Lädt einen Binärstring in das Register.
        Der Binärstring muss genau num_qubits lang sein.
        """
        if len(binary_str) != self.num_qubits:
            raise ValueError(f"Input-Länge ({len(binary_str)}) passt nicht zu {self.num_qubits} Qubits!")
...
def execute_quantum_command(cpu, reg_index, command_line):
    ...
    try:
        if op == "LOAD":
            # Lade einen vorgegebenen Binärstring aus der QCP-Datei.
            binary_input = tokens[1]
            reg.load_binary(binary_input)
            print(f"Register R{reg_index} geladen mit: {binary_input}")
        elif op == "READ":
            # READ: Fordert den Benutzer interaktiv zur Eingabe eines Binärstrings auf.
            binary_input = input(f"Register R{reg_index} - Bitte Binärstring eingeben (Länge {reg.num_qubits}): ")
            reg.load_binary(binary_input)
            print(f"Register R{reg_index} geladen mit: {binary_input}")
        elif op == "H":
            qubit = int(tokens[1])
